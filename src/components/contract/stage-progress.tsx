...
export function ContractStageProgress({ status, currentRound }: Props) {
  ...
  if (status === "PENDING_BU_REVISION") {
    return (
      <div className="flex flex-col gap-2">
        <ol className="flex w-full items-start gap-1 overflow-x-auto px-1 py-3 sm:gap-0">
          {STEPS.map((step, i) => {
            const reviewIdx = STEPS.findIndex((s) => s.status === "IN_LEGAL_REVIEW");
            const isComplete = i < reviewIdx;
            const isCurrent = false;
            const isFuture = i >= reviewIdx;
            return (
              <li
                key={step.status}
                className={cn(
                  ...
                  isComplete ? "after:bg-slate-900 dark:after:bg-slate-100" : "after:bg-slate-200 dark:after:bg-slate-700",
                )}
              >
                <div
                  className={cn(
                    "z-10 flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 text-[11px] font-semibold transition-colors",
                    isComplete && "border-slate-900 bg-slate-900 text-white dark:border-slate-100 dark:bg-slate-100 dark:text-slate-900",
                    isCurrent && "border-blue-600 bg-blue-50 text-blue-700 ring-4 ring-blue-100",
                    isFuture && "border-slate-300 bg-white text-slate-400 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-500",
                  )}
                >
                  {isComplete ? (
                ...
                <span className={cn("text-center text-[10px] leading-tight sm:text-xs", isComplete && "text-slate-700", isFuture && "text-slate-400")}>
        ...
        <div className="flex items-center justify-between gap-3 rounded-md border border-yellow-300 bg-yellow-50 px-4 py-3 text-sm text-yellow-900 dark:border-yellow-700/40 dark:bg-yellow-950/30 dark:text-yellow-200">
          <span className="font-medium">Sent back to BU owner — awaiting resubmission</span>
          {currentRound > 0 ? (
            <span className="text-xs font-medium opacity-70">Round {currentRound}</span>
          ) : null}
        </div>
      </div>
    );
